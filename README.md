# couparvi — Phase 1 + Phase 2 + Phase 3

쿠팡 파트너스 쇼츠 자동 제작 파이프라인. 현재 구현 범위는 **Phase 1(상품발굴 & 후기수집)**,
**Phase 2(AI 후기분석 & 대본작성)**, **Phase 3(미디어 파이프라인)**다. 전체 배경은
`docs/00_project_overview.md`, 규칙은 `AGENTS.md` 참조.
...
`migrations/001_init.sql`, `migrations/002_phase2.sql`, `migrations/003_phase3.sql`을
순서대로 Supabase SQL Editor에 붙여넣어 실행하거나, `psql`로 적용한다.
...
렌더링(Phase 3)은 별도 워커 프로세스가 `render_jobs`를 폴링해 처리한다:
...
썸네일 생성, 업로드(홀드 큐), 대시보드 UI, 쿠팡 페이지 크롤링(절대 금지) —
`docs/phase1_spec.md`, `docs/phase2_spec.md`, `docs/phase3_spec.md`, `docs/phase4_spec.md` 참조.
