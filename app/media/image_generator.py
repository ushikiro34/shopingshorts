...
def _build_prompt(
    ...
    if has_character_reference:
        character_instruction = (
            ...
            f"단, 인종·피부톤·얼굴형·이목구비·나이대·머리색은 절대로 바뀌면 안 된다 — 이건 "
            f"'비슷한 분위기의 다른 사람'이 아니라 '정확히 이 사람'이어야 한다. 예를 들어 두 "
            f"번째 참고 이미지 속 인물이 서양인이면 이번 장면의 인물도 반드시 서양인이어야 "
            f"하고, 동양인이면 반드시 동양인이어야 한다 — 인종이 바뀌는 것은 자세나 구도가 "
            f"똑같은 것보다 훨씬 심각한 실패로 간주한다. 사람이 꼭 필요하지 않은 장면(예: 제품 "
