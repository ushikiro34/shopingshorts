"""3호 직원 — 대본 프롬프트: 구조(고정 6단계) + 톤(7종 가변) 이중 프롬프트.
...
# 단계별 권장 초는 실제 쇼츠 대본 작성 가이드(후킹 3초/문제제시 7초/해결 10초/결과 5초/CTA 5초,
# 합계 30초)를 참고해 우리 6필드 구조에 맞게 배분했다. 정확히 맞출 필요는 없는 "가이드"이고,
# 특정 단계가 지나치게 길거나 부실해지지 않게 균형을 잡는 기준으로만 쓴다.
STRUCTURE_INSTRUCTION = """구조는 정확히 6단계로 고정한다: 공감 -> 감정 -> 문제제기 -> 해결 -> 결과 -> 상품.
